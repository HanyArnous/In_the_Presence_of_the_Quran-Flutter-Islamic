...
class _SurahListPageState extends State<SurahListPage> {
  ...
  getStarredVerse() async {
    ...
    if (savedData != null && savedData.isNotEmpty) {
      ...
      if (decoded is List) {
        starredVerses = Set<String>.from(decoded);
      }
    }
    setState(() {});
  }
  ...
  removeStarredVerse(String verseToRemove) async {
    ...
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final encoded = json.encode(starredVerses.toList());
    await prefs.setString("starredVerses", encoded);
                              ...
                              .map((e) => EasyContainer(
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      Text(
                                          "${convertToArabicNumber((int.parse(e.split("-")[1]).toString()))} ",
                                          style: TextStyle(
                                              fontSize: 24.sp,
                                              color: Colors.blueAccent,
                                              fontFamily:
