...
class _PlayerBarState extends State<PlayerBar> with AutomaticKeepAliveClientMixin {
  DateTime? _startTime; // لتتبع وقت بدء الاستماع
  bool _isProcessingAction = false; // حارس لمنع التكرار في التشغيل/الإيقاف
  ...
  void initState() {
    super.initState();
    // استعادة وقت البدء المحفوظ عند إعادة فتح التطبيق
    _restoreStartTime();
    addFavorites();
    BackButtonInterceptor.add(myInterceptor);
  }

  // استعادة وقت البدء من التخزين
  void _restoreStartTime() {
    final startTimeString = getValue("audio_start_time");
    if (startTimeString != null) {
      _startTime = DateTime.parse(startTimeString);
    }
  }

  // حفظ وقت البدء في التخزين
  void _saveStartTime() {
    if (_startTime != null) {
      updateValue("audio_start_time", _startTime!.toIso8601String());
    }
  }
  ...
  void _playAudio(AudioPlayer player) {
    if (_isProcessingAction) return; // منع التكرار
    
    _isProcessingAction = true;
    _startTime = DateTime.now(); // تسجيل لحظة البدء
    _saveStartTime(); // حفظ الوقت في التخزين
    player.play();
    
    // إعادة تعيين الحارس بعد فترة قصيرة
  ...
  void _pauseAudio(AudioPlayer player) {
    ...
    _isProcessingAction = true;
    if (_startTime != null) {
      // حساب مدة الجلسة وتسجيلها في الإحصائيات
      final sessionDuration = DateTime.now().difference(_startTime!).inMinutes;
      if (sessionDuration >= 1) {
        final dateKey = DateFormat('yyyy-MM-dd').format(DateTime.now());
        int currentMin = getValue("$dateKey-quran_listening-count") ?? 0;
        updateValue("$dateKey-quran_listening-count", currentMin + sessionDuration);
        
        int totalMin = getValue("quran_listening-totalCount") ?? 0;
        updateValue("quran_listening-totalCount", totalMin + sessionDuration);
      }
      
      _startTime = null; // إعادة ضبط الوقت لجلسة جديدة
      _clearStartTime(); // مسح الوقت من التخزين
    }
    player.pause();
  ...
  Widget build(BuildContext context) {
    super.build(context); // دعم الـ mixin للحفاظ على الحالة
    return Builder(
            ...
            if (state is PlayerBlocPlaying) {
              return BlocBuilder<PlayerBarBloc, PlayerBarState>(
                ...
                builder: (context, statee) {
                  // print(statee); // ❌ تمت إزالته لتحسين الأداء
                  if (statee is PlayerBarHidden) {
                    return Positioned(
                        bottom: (MediaQuery.of(context).padding.bottom) + 25.h,
                        right: 25.w,
                        child: FadeInRight(
                          child: FadeInUp(
                            child: StreamBuilder(
                                              ...
                                              builder: (context, snapshot) {
                                                ...
                                                if (snapshot.hasData) {
                                                  ...
                                                  if (metadata.artist != null) {
                                                    currentReciterName = metadata.artist!;
                                                  } else if (metadata.album != null) {
                                                    // استخراج اسم القارئ من album إذا كان يحتوي على معلومات القارئ
                                                    String albumInfo = metadata.album!;
                                                    if (albumInfo.contains("القارئ")) {
                                                      currentReciterName = albumInfo.split("القارئ").last.trim();
                                                    }
                                                  }
                                      ...
                                      child: Container(
                                        ...
                                        child: SafeArea(
                                          ...
                                          children: [
                                            ...
                                            // if (isPlaylistShown = false)
                                            // استدعاء الأزرار مع ربطها بالدوال التي تحسب الوقت
                                            ControlButtons(
                                              state.audioPlayer,
                                              onPlay: () =>
                                                  _playAudio(state.audioPlayer),
                                              onPause: () => _pauseAudio(
                                                  state.audioPlayer),
                                              onTrackChange: () => _pauseAudio(
                                                  state.audioPlayer),
                                            ),
                                            // if (isPlaylistShown == false)
...
class ControlButtons extends StatelessWidget {
  ...
  final VoidCallback onPlay; // إضافة هذا
  final VoidCallback onPause; // إضافة هذا
  final VoidCallback onTrackChange; // إضافة هذا لتتبع تغيير السورة
        ...
        StreamBuilder<SequenceState?>(
          ...
          builder: (context, snapshot) => IconButton(
            ...
            onPressed: player.hasPrevious
                ? () {
                    // تسجيل الدقائق قبل الانتقال للسورة السابقة
                    onTrackChange();
                    player.seekToPrevious(); // ✅ إضافة الأقواس لاستدعاء الدالة
                  }
                : null,
          ),
        ),
          ...
          builder: (context, snapshot) {
            ...
            if (processingState == ProcessingState.loading ||
              ...
              return IconButton(
                ...
                color: Colors.white,
                onPressed:
                    onPlay, // 👈 استخدمنا الدالة الممرة بدلاً من player.play
              );
              ...
              return IconButton(
                ...
                color: Colors.white,
                onPressed:
                    onPause, // 👈 استخدمنا الدالة الممرة بدلاً من player.pause
              );
            } else {
              return IconButton(
        ...
        StreamBuilder<SequenceState?>(
          ...
          builder: (context, snapshot) => IconButton(
            ...
            onPressed: player.hasNext
                ? () {
                    // تسجيل الدقائق قبل الانتقال للسورة التالية
                    onTrackChange();
                    player.seekToNext(); // ✅ إضافة الأقواس لاستدعاء الدالة
                  }
                : null,
          ),
        ),
