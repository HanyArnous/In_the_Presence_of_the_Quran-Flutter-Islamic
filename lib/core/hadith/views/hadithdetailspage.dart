...
class _HadithDetailsPageState extends State<HadithDetailsPage> {
  ...
  getHadithData() async {
    ...
    final arKey = "hadith-ar-${widget.id}";
    final otherKey = "hadith-${widget.locale}-${widget.id}";

    final cachedAr = prefs.getString(arKey);
    final cachedOther = prefs.getString(otherKey);

    if (cachedAr != null && cachedOther != null) {
      try {
        final arData = json.decode(cachedAr);
        final otherData = json.decode(cachedOther);

        hadithAr = Hadith.fromJson(arData);
        hadithOtherLanguage = otherData;
    ...
    try {
      ...
      final responseOther = responses[0];
      final responseAr = responses[1];

      hadithOtherLanguage = responseOther.data;
      hadithAr = Hadith.fromJson(responseAr.data);

      await prefs.setString(arKey, json.encode(responseAr.data));
      await prefs.setString(otherKey, json.encode(responseOther.data));
    } catch (e) {
      setState(() {
        isLoading = false;
      });
