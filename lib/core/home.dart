...
import 'package:nabd/core/statistics/statistics_page.dart';
import 'package:nabd/core/downloads/downloads_page.dart';
...
import 'package:nabd/main.dart' show playerbarBloc;
...
class _HomeState extends State<Home>
  ...
  getAndStoreRadioData() async {
    ...
    try {
      if (context.locale.languageCode == "ms") {
        response =
            await Dio().get('https://mp3quran.net/api/v3/radios?language=eng');
      } else {
        response = await Dio().get(
            'https://mp3quran.net/api/v3/radios?language=${context.locale.languageCode == "en" ? "eng" : context.locale.languageCode}');
      }
  ...
  void _saveHomeVerseForCurrentHour(int s, int v) {
    updateValue("homeRandomVerse",
        {"hourKey": _currentHourKey(), "sura": s, "ayah": v});
  }

  void _applyStableHomeVerse() {
    final saved = getValue("homeRandomVerse");
    final key = _currentHourKey();
    if (saved is Map && saved["hourKey"] == key) {
      final s = int.tryParse(saved["sura"].toString());
      final v = int.tryParse(saved["ayah"].toString());
      if (s != null && v != null) {
        setState(() {
          suranumber = s;
          verseNumber = v;
      ...
      _saveHomeVerseForCurrentHour(suranumber, verseNumber);
    }
  }

  void _pauseResumeAudio() {
    playerbarBloc.add(TogglePlayPauseEvent());
    qurapPagePlayerBloc.add(PausePlaying());
  }

  void _stopAllAudio() {
    playerbarBloc.add(StopAudioEvent());
    qurapPagePlayerBloc.add(KillPlayerEvent());
  }

  void _openHomeHadithDetails() {
    final hadith = hadithes[indexOfHadith];
        ...
        builder: (context) {
          return Container(
            decoration: BoxDecoration(
              color: getValue("darkMode") ? quranPagesColorDark : Colors.white,
...
//         : null;
  ...
  getAndStoreRecitersData() async {
    ...
    try {
      if (context.locale.languageCode == "ms") {
        response =
            await Dio().get('https://mp3quran.net/api/v3/reciters?language=eng');
        response2 =
            await Dio().get('https://mp3quran.net/api/v3/moshaf?language=eng');
        response3 =
            await Dio().get('https://mp3quran.net/api/v3/suwar?language=eng');
      } else {
        response = await Dio().get(
            'https://mp3quran.net/api/v3/reciters?language=${context.locale.languageCode == "en" ? "eng" : context.locale.languageCode}');
        response2 = await Dio().get(
            'https://mp3quran.net/api/v3/moshaf?language=${context.locale.languageCode == "en" ? "eng" : context.locale.languageCode}');
        response3 = await Dio().get(
            'https://mp3quran.net/api/v3/suwar?language=${context.locale.languageCode == "en" ? "eng" : context.locale.languageCode}');
      }
                                                                  ...
                                                                  child:
                                                                      IconButton(
                                                                          onPressed:
                                                                            ...
                                                                            _saveHomeVerseForCurrentHour(suranumber,
                                                                                verseNumber);
                                                                          },
                                                                          icon:
                                                                              Icon(
                                                                    ...
                                                                    child: IconButton(
                                                                        onPressed: () async {
                                                                          ...
                                                                          await Clipboard.setData(
                                                                              ClipboardData(text: "$verse \nسورة $suraName"));
                                                                          ScaffoldMessenger.of(context)
                                                                              .showSnackBar(const SnackBar(content: Text("تم النسخ")));
                                                                        },
                                                                        icon: Icon(Iconsax.copy, color: Colors.white, size: 18.sp))),
                                                                Container(
                                                                    decoration: BoxDecoration(
                                                                        shape: BoxShape
                                                                            .circle,
                                                                      ...
                                                                      await Navigator.push(
                                                                          ...
                                                                          CupertinoPageRoute(
                                                                              builder: (builder) => BlocProvider(
                                                                                    ...
                                                                                    child: QuranDetailsPage(shouldHighlightSura: false, pageNumber: page, jsonData: widgejsonData, quarterJsonData: quarterjsonData, shouldHighlightText: false, highlightVerse: ""),
                                                                                  )));
                                                                    },
                                                                    child: Text(
